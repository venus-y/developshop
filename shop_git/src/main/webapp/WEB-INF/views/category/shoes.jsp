...
<body>
	...
	<c:if test="${ph.prevPage}">
		<a href="<c:url value="/product/categorySet?category_code=3&viewName=shoes&page=${ph.startPage-1}&pageSize=${ph.pageSize}"/>">&lt</a>
	</c:if>

	<c:forEach var="i" begin="${ph.startPage}" end="${ph.endPage}">
		<a href="<c:url value="/product/categorySet?category_code=3&viewName=shoes&page=${i}&pageSize=${ph.pageSize}"/>">${i}</a>
	</c:forEach>
	
	<c:if test="${ph.nextPage}">
		<a href="<c:url value="/product/categorySet?category_code=3&viewName=shoes&page=${ph.endPage+1}&pageSize=${ph.pageSize}"/>">&gt</a>
	</c:if>
	</div>
</body>
</html>